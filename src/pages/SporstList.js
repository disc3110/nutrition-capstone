...
const SportsList = () => {
  ...
  const thisSports = currentGroup[0].relationships.sports.data.map(
    (sport) => sports.filter((allsport) => allsport.id === sport.id)[0],
  );
  ...
  return (
    <>
      ...
      <p className="subtitle">
        {' '}
        { `LIST OF ${currentGroup[0].attributes.name.toUpperCase()}` }
        {' '}
      </p>
      <div>
        {' '}
        {renderSports(thisSports)}
        {' '}
      </div>
    </>
  );
